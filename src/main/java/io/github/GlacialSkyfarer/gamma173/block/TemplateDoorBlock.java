...
public class TemplateDoorBlock extends TemplateBlock {
    ...
    public TemplateDoorBlock(Identifier identifier, Material ignoredMaterial) {
    ...
    public BlockState getPlacementState(ItemPlacementContext context) {
        return getDefaultState().with(TOP, false).with(FACING, context.getHorizontalPlayerFacing().rotateYCounterclockwise()).with(FLIPPED, isFlipped(context)).with(OPEN, false);
    }
    ...
    public void updateBoundingBox(World world, int x, int y, int z) {
        setDoorBoundingBox(world,x,y,z);
    }

    protected void setDoorBoundingBox(World world, int x, int y, int z) {

        if (world.getBlockId(x,y,z)!=id) return;
    ...
    public void onPlaced(World world, int x, int y, int z, LivingEntity placer) {
        ...
        setDoorBoundingBox(world,x,y,z);
        setDoorBoundingBox(world,x,y+1,z);
    }
